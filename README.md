...
- **9 条跨课程知识关联**注入：C语言（指针/位运算/数组）× 电路分析（分压/波形）× STM32（外设编程），体现"编程思维 → 电路建模 → 嵌入式实现"链路
- **学科元数据**（`/learning-path/courses`）：3 门课程明确学科领域（计算机科学与技术 / 电子信息工程）、核心环节（教学实践/实验实训/自主学习）、跨课程关联统计
- **跨学科综合路径**（`/learning-path/cross-discipline`）：目标知识点（如 STM32 定时器PWM）的前置依赖链可跨越 3 门课程，DAG 可视化按学科着色
- 路径算法（ADPP）本身跨课程兼容，仅需数据层注入关联

### 故障诊断实验（电路仿真器 · 故障实验按钮）

- 3 个故障实验模板：分压电路 R2 断路 / R1 短路 / 电流源电路 R1 断路
- 实验闭环：加载故障电路 → 自动仿真观察异常测量值 → 选择故障原因 → 本地规则即时判定 → AI 详细评估（后端诊断模式）
- "AI+虚拟实验实训"新范式：传统仿真只给数值，LearnLab 进一步给出故障原理讲解与排查方法

### 试点数据分析（教师端 · 试点数据分析页）

- 自动聚合 5 维度数据：学习行为（时长/记录/掌握度）、测验成绩（含前后测对比）、学习趋势分布、实验参与、功能使用 Top
- 一键生成 AIC"应用效果"评分项所需的效果验证数据
- 时间范围切换（7/14/30 天），支持单学生/全班视角

### 实验行为采集与功能使用统计

- **实验行为日志**（`experiment_logs`）：电路仿真 / 故障诊断 / STM32 仿真行为自动埋点（运行、诊断提交）
- **功能使用频率**（`/monitoring/feature-usage`）：38 项功能映射，按学生统计使用频次与平均耗时，试点分析基础数据
- 权限：学生可查自己，教师可查全班/任意学生

### 多模型注册与自动降级（LLMFactory 升级）

- 5 家提供商注册：讯飞星火 / DeepSeek / 智谱GLM / OpenAI / 小米MiMo（原仅注册星火一家，文档与代码对齐修复）
- **FailoverLLM 自动降级链**：主提供商失败自动切换备用（未配置 Key 的自动跳过）
- 主提供商未配置时自动选择已配置的提供商；全未配置给出清晰报错
- 日志按真实 provider 记录（修复硬编码 bigmodel/openai 标签）

### 其他增强

- **STM32 仿真器 AI 分析**：数字电路仿真结果（引脚状态/仿真日志）接入 AI 电路分析（此前仅模拟电路仿真器有此功能）
- 监控中间件自动提取 student_id（功能使用统计基础）

### 实验实训与试点支撑（优化批次）

- **STM32 实验实训**（学习中心·实验实训 Tab）：7 个实验完整交互（实验目标/元件清单/步骤勾选/原理），完成度自动采集
- **故障实验报告生成**：一键生成 Markdown 实验报告（实验信息/现象/诊断结论/AI评估），支持下载与复制
- **STM32 实验报告**：实验完成生成标准实验报告（目标/元件/步骤/原理 → 下载 .md）
- **跨学科综合实战项目**（学习路径页）："智能温控风扇""呼吸灯与按键交互"2 个跨学科实战项目，任务勾选+完成上报
- **班级维度**：学生班级（class_id）、班级列表、**班级对比页**（教师端：人数/均分/时长/活跃度对比，试点实验组vs对照组数据源）
- **试点报告 Markdown 导出**：试点数据分析一键导出 Markdown（参赛文档素材），支持按班级过滤
- **LandingPage 学科交叉展示区**：三学科卡片（计算机/电子信息/交叉）+ "编程思维→电路建模→嵌入式实现"链路图
- **AI 服务降级**：电路分析/知识图谱构建在 LLM 不可用时自动降级（引导提示/本地图谱兜底），不再 500
- **防幻觉全挂接**：引用核查+结果合并（规则守卫）+ 低质量输出 LLM 自我纠错
