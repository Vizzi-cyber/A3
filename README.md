...
│   │   │   └── ppt_generator.py   # PPT 自动生成服务（python-pptx）
│   │   └── main.py                # 应用入口
│   ├── .env.example               # 环境变量模板
...
#### 多智能体协作
1. ✅ 智能体基类框架
2. ✅ 课程设计师智能体（Supervisor 总控调度）
3. ✅ 画像师智能体（Profiler，6维学生画像构建）
4. ✅ 资源生成师智能体（ResourceGen，自适应难度资源生成）
5. ✅ 路径规划师智能体（PathPlanner，DAG 路径规划）
6. ✅ 辅导助手智能体（Tutor，苏格拉底式问答 + 学习状态感知）
7. ✅ LangGraph 多智能体工作流编排
8. ✅ Agent 工作流可视化（实时展示智能体协作状态、任务进度、执行日志）

#### 个性化学习
9. ✅ 学生画像 API（6维度：知识基础、认知风格、薄弱环节、兴趣领域、学习习惯、情感状态）
10. ✅ 学习路径 API（DAG 路径规划 + 动态调整）
11. ✅ 智能辅导 API（苏格拉底式问答 + WebSocket 流式 + 学习状态自动检测）
12. ✅ 艾宾浩斯复习提醒（基于遗忘曲线的智能复习推荐）

#### 内容生成
13. ✅ 资源生成 API（对接 LangGraph + 真实智能体，文档/题目/思维导图/代码）
14. ✅ PPT 自动生成（AI生成深度学习PPT，含代码、算法步骤、复杂度分析、代码走读）
15. ✅ 代码在线编译运行（Python AST 安全检查 + C 语言 gcc 沙箱执行）
16. ✅ 文生图（AI 生成学习插图）
17. ✅ OCR 识图（图片文字识别）

#### 游戏化系统
18. ✅ 知识树成长系统（可视化知识树，学习越多树越茂盛，等级提升动画）
19. ✅ 经验等级系统（后端统一配置 /level-config，10级：初学者 → 传奇，前后端共享计算逻辑）
20. ✅ 学习挑战系统（任务地图，难度递增，完成获得奖励）
21. ✅ 六维排行榜（积分/连续/掌握/测验/AI协作/进步 多维度排名）
22. ✅ 游戏化基础（积分、成就、任务、排行榜）

#### 数据分析
23. ✅ Dashboard 仪表盘（统计卡片、学习趋势分析、3D 滚动旅程、Agent 工作流面板、成长时间轴）
24. ✅ 趋势分析与预测（算法模型评估学习效果、预测掌握度、预警干预）
25. ✅ 成长时间轴（个性化里程碑展示：能力提升/预警/解锁成就）

#### 算法可视化
26. ✅ 算法动画（冒泡排序、快速排序、BFS/DFS、链表操作的 Canvas 动画）
27. ✅ 伪代码高亮（动画同步高亮当前执行行 + AI 讲解）

#### 系统能力
28. ✅ 讯飞星火 / DeepSeek / OpenAI 多模型支持（统一接口，一键切换）
29. ✅ 前端页面完整实现（Dashboard、资源中心、学习路径、个人空间、知识树、挑战、排行榜等）
30. ✅ JWT 认证系统（注册/登录/Token 刷新）
31. ✅ 防幻觉机制（JSON 校验、代码语法检查、引用溯源）
32. ✅ 内容安全过滤（敏感词过滤、Prompt 安全加固）
33. ✅ 学习日志与反思（康奈尔笔记、费曼学习法）
34. ✅ 知识点体系（C语言 16 个知识点的内容库）
35. ✅ 收藏夹（资源收藏管理）
36. ✅ Playwright E2E 测试（22个 API 测试用例，全量通过）
