...
**AI Learning System** 是一个面向C语言和电路分析学科的个性化AI学习平台。系统通过12个AI智能体协作，为学生提供个性化学习路径规划、智能辅导、游戏化激励、知识库管理、错误诊断、项目协作等功能。支持学生端和教师端双角色。
...
### 创新点三：智能体反思循环（Self-Reflection Loop）
...
| **工程化** | 135+API，38个E2E测试 | FastAPI + Playwright + Docker部署 |
...
│  12个智能体 + 33个API路由模块 + 22个数据模型 + 4个学习算法         │
└─────────────────────────────────────────────────────────────┘
                              │
┌─────────────────────────────────────────────────────────────┐
│                    大模型接入层                                │
...
| 技术 | 版本 | 用途 |
|------|------|------|
| **FastAPI** | >=0.115.0 | 高性能异步Web框架，33个API路由模块 |
...
| **KaTeX** | ^0.17.0 | 数学公式渲染 |
| **Playwright** | ^1.59.1 | E2E测试（38个测试用例） |
| **Sentry** | ^10.50.0 | 错误监控 |
...
│   │   └── main.tsx               # React入口
│   ├── e2e/                       # Playwright E2E测试（4个文件，38个用例）
