    ...
    <div className="space-y-8">
      ...
      >
        ...
        <div className="relative space-y-32 py-20" style={{ zIndex: 1 }}>
          ...
          <div
            ref={setSceneRef(0)}
            className="flex justify-start px-4 md:px-12"
            ...
            <div className="w-full max-w-xl">
              <SectionCard bodyPadding="28px">
                ...
                >
                  ...
                  <Tag className="rounded-full border-0 bg-primary-50 text-primary text-xs ml-auto">
                    共 {pathNodesState.length} 个阶段
                  </Tag>
                  {pathExpanded ? (
                    <UpOutlined className="text-slate-400 text-xs" />
                  ) : (
                    ...
                    <div className="space-y-6">
                      {pathNodesState.map((node) => (
                        ...
                        >
                          ...
                          <div
                            className="flex-1 p-4 rounded-xl bg-white border border-slate-100 hover:border-slate-200 hover:shadow-card transition-all cursor-pointer"
                            onClick={() => navigate("/learning-path")}
                          >
                          ...
                          {nextNode && (
                            <div className="flex items-center gap-3 p-3 rounded-xl bg-white border border-slate-100">
                              <div
                                ...
                                style={{
                                  background: statusColors[nextNode.status],
                                }}
                              >
                                {nextNode.status === "in-progress" ? (
                                  <ClockCircleOutlined />
                                ) : (
                                  <EnvironmentOutlined />
                                )}
              ...
              >
                <div className="flex flex-wrap gap-3">
                  {challenges.map((c, idx) => (
                    <div
                      key={idx}
                      className={`flex items-center gap-3 px-4 py-3 rounded-xl border transition-all ${c.completed ? "bg-slate-50 border-slate-100 opacity-60" : "bg-white border-slate-200 hover:border-primary hover:shadow-card cursor-pointer"}`}
                    >
                      <div
                        className={`w-8 h-8 rounded-lg flex items-center justify-center text-white text-sm shrink-0 ${c.completed ? "bg-slate-300" : "bg-warning"}`}
                      >
                ...
                <Spin spinning={summaryLoading}>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    {recommendations.length === 0 && (
                      <div className="text-slate-400 text-sm col-span-2 text-center py-4">
                        暂无推荐资源
                      </div>
                    )}
                  </div>
              ...
              <SectionCard
                title={
                  <Space>
                    ...
                    <Badge
                      count="AI"
                      style={{ backgroundColor: "#4f46e5", fontSize: 10 }}
                    />
                  </Space>
                }
                extra={
              ...
              >
                <div className="grid grid-cols-4 gap-3">
                  {badgesState.map((b, idx) => (
                    <div
                      key={idx}
                      className={`flex flex-col items-center gap-2 p-3 rounded-xl transition-all ${b.unlocked ? "bg-slate-50" : "bg-slate-50 opacity-40"}`}
                    >
                      <div
                        className="w-10 h-10 rounded-full flex items-center justify-center text-white text-lg"
                        style={{ background: b.unlocked ? b.color : "#cbd5e1" }}
                      >
                        {b.icon}
                      </div>
              ...
              >
                <div className="flex flex-wrap gap-2">
                  <Tooltip title="已适配文心一言">
                    <Tag
                      icon={<FlagFilled />}
                      className="rounded-full border-0 bg-blue-50 text-blue-600 text-xs"
                    >
                      文心一言
                    </Tag>
                  </Tooltip>
                  <Tooltip title="已适配通义千问">
                    <Tag
                      icon={<FlagFilled />}
                      className="rounded-full border-0 bg-purple-50 text-purple-600 text-xs"
                    >
                      通义千问
                    </Tag>
                  </Tooltip>
                  <Tooltip title="已适配国产算力平台">
                    <Tag
                      icon={<FlagFilled />}
                      className="rounded-full border-0 bg-emerald-50 text-emerald-600 text-xs"
                    >
                      昇腾 NPU
                    </Tag>
                  </Tooltip>
                  <Tooltip title="RAG 检索增强">
                    <Tag
          ...
          >
            <div className="w-full max-w-lg text-center py-16">
              ...
              <Typography.Title
                level={4}
                className="!m-0 text-slate-800 font-bold"
              >
                学习之路，永无止境
              </Typography.Title>
              <Typography.Text className="text-slate-400 text-sm block mt-2">
                继续探索，下一站的风景更精彩
              </Typography.Text>
              <Button
                type="primary"
            ...
            {kgNodes.length > 0 ? (
              <div className="grid grid-cols-3 gap-3">
                {kgNodes.map((node) => (
                  <div
                    key={node}
                    className="p-3 rounded-lg bg-white border border-slate-200 text-center text-sm text-slate-700 hover:border-primary hover:shadow-sm transition-all cursor-pointer"
                  >
                    {node}
                  </div>
                ))}
