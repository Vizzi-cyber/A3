...
const ResourceDetail: React.FC = () => {
  ...
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
    {
      role: "ai",
      content: "你好！我是你的 AI 学习助手。关于本知识点有任何疑问，随时问我。",
      agent: "辅导助手",
    },
  ]);
            ...
            <div className="flex items-center gap-3 p-4 rounded-xl bg-slate-50 border border-slate-100">
              ...
              <div>
                ...
                <div className="text-xs text-slate-400">
                  答对 {Math.round((quizScore / 100) * questions.length)}/
                  {questions.length} 题
                </div>
              </div>
            </div>
                ...
                <List.Item className="border border-slate-100 rounded-xl p-5 mb-4 hover:shadow-card transition-all">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <Typography.Text className="font-medium text-slate-800 block mb-3">
                        {q.content}
                      </Typography.Text>
                      <Radio.Group
                      ...
                      {quizSubmitted[q.q_id] !== undefined && (
                        <div
                          className={`mt-3 text-xs p-3 rounded-lg ${quizSubmitted[q.q_id] ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}
                        >
                          <strong>解析：</strong>
                          {q.explanation ||
                            (quizSubmitted[q.q_id]
                              ? "回答正确！"
                              : "回答错误，请查看正确答案。")}
                        </div>
                      )}
      ...
      <div className="flex items-center justify-between">
        ...
        <Space>
          <Tooltip title="完成学习后可获得经验值">
            <Tag
              icon={<FireOutlined />}
              className="rounded-full border-0 bg-amber-50 text-amber-600 text-xs"
            >
              完成后 +20 XP
            </Tag>
          </Tooltip>
