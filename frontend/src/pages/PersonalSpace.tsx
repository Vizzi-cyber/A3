  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        // Review topics based on weak areas
        const weak = pRes.data?.data?.weak_areas || ['指针与内存', '递归函数']
        setReviewTopics(
          weak.map((w: string, i: number) => ({
            topic: w,
            retention: [45, 62, 55, 78][i % 4],
            nextReview: i % 2 === 0 ? '今天' : '明天',
          }))
        )
      } catch (e) {
        console.error(e)
                ...
                <div className="space-y-4 text-slate-600 leading-relaxed">
                  <div className="p-5 rounded-xl bg-slate-50 border border-slate-100">
                    ...
                    <p className="mb-2">
                      ...
                      {weakAreas.join('、') || '暂无'}，系统已自动增加相关练习推送和可视化讲解。
                    </p>
                    <p>
                      <strong className="text-slate-800">兴趣方向：</strong>{' '}
                      {profile?.interest_areas?.join('、') || 'C语言程序设计与系统开发'}。
                    </p>
                  </div>
                </div>
                <Button type="primary" className="mt-5 rounded-lg bg-primary">
                  手动修正画像
                </Button>
              </div>
            ),
