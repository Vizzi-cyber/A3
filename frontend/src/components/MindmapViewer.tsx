...
function treeToGraph(data: MindmapData): { nodes: MindmapNode[]; edges: MindmapEdge[] } {
  ...
  const queue: { name: string; parentId: string; children?: MindmapTreeNode[] }[] = [];
...
const MindmapViewer: React.FC<MindmapViewerProps> = ({
  ...
  const renderGraph = useCallback(() => {
    ...
    const nodes: MindmapNode[] = graphData.nodes.map((n, i) => ({ ...n, index: i }));
