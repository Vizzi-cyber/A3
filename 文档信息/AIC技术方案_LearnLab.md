...
- 标准化 AgentMessage 通信协议 + 反思循环（执行-评估-修正，最多 3 次；由 `REFLECTION_ENABLED` 配置启用，评审演示时可现场开启）
...
**算法接线（非孤立演示）**：IRT θ 接入效果评估掌握度（Φ(θ)·100 百分位，`mastery_detail` 可溯源）、IRT b 接入 ADPP 学习成本模型与资源匹配难度适配（`difficulty_source` 可区分）、GKT 接入路径掌握度传播、MAB 接入路径调整/选题/匹配三链路并支持收益闭环回传、FSRS 接入效果评估记忆小节并把到期知识点自动插入学习路径头部复习阶段（`review_source=fsrs_due` 可溯源）。**算法状态同时注入 tutor / error_catcher / resource_generator 三处智能体 prompt**（`build_ai_engine_context` 共享构建，静默降级）。**趋势分析 6 因子权重由历史数据学习**（L2 正则逻辑回归，标签=随后一周掉队/中断），输出掉队预警概率，未训练时回退人工先验。
...
- **AI 故障诊断实验**：3 个故障模板（断路×2 / 短路×1，错值以极端阻值注入实现）→ 学生观察异常测量 → 选择原因 → 本地规则判定 + AI 评估（原理讲解/排查方法）
...
输入过滤 → Prompt 加固 → 结构校验 → 代码校验 → 引用溯源 → LLM 自我纠错；①-⑤ 常驻主管线（`apply_guards` 规则守卫零成本），⑥ 自我纠错随反思循环生效（`REFLECTION_ENABLED` 配置启用）。
...
- 测试说明书（12 章，系统测试佐证）
- 试点知情同意书（试点前发放）
- 演示视频（3-5 分钟，待录制：：覆盖跨学科路径、故障实验、算法对照面板、教师报告）
