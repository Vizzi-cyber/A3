...
function TrendWave({ intensity }: { intensity: number }) {
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const y =
        Math.sin(z * 1.5 + t * 1.8) * 0.22 * (0.5 + intensity) +
    ...
    <group ref={group} position={[0, 0.9, 0]}>
      ...
      {GRAPH_NODES.map((n, i) => {
        ...
        return (
          <Line
            key={`cl${i}`}
            points={[new THREE.Vector3(0, 0, 0), new THREE.Vector3(...p)]}
            color={n.color}
            lineWidth={1}
            transparent
            opacity={0.35}
      ...
      {GRAPH_NODES.map((n) => {
        ...
        return (
          <group
            key={n.name}
            position={[Math.cos(a) * n.radius, n.y, Math.sin(a) * n.radius]}
          >
            <mesh
              onPointerOver={(e) => {
                e.stopPropagation();
            ...
            {hovered === n.name && (
              <Html center distanceFactor={10} pointerEvents="none">
                <div
                  className="whitespace-nowrap px-2.5 py-1 rounded-md text-[11px]"
                  style={{
                    background: "rgba(255,255,255,0.92)",
                    border: `1px solid ${n.color}77`,
                    color: "#334155",
                    boxShadow: `0 0 14px ${n.color}44`,
                  }}
                >
                  {n.name}
                </div>
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <span
              ...
              style={{
                background: `${def.accent}1c`,
                color: def.accent,
                boxShadow: `0 0 12px ${def.accent}33`,
              }}
            >
              {def.icon}
            </span>
  ...
  const panels: PanelDef[] = [
    {
      ...
      render: (d) => (
        <div className="flex items-center gap-3">
          <AnimatedRing
            percent={Math.min(100, Math.round((d.masteredKps / 20) * 100))}
            size={54}
            strokeWidth={5}
            color="#818cf8"
          ...
          >
            <span className="text-xs font-bold text-slate-800">
              <CountUp value={d.masteredKps} duration={1.2} />
            </span>
          </AnimatedRing>
          <div
      ...
      render: (d) => (
        <div>
          <div className="text-2xl font-bold text-slate-800">
            <CountUp value={d.weeklyHours} duration={1.4} decimals={1} />
          ...
          <div
            ...
            style={{ color: "#94a3b8" }}
          >
            <span>🔥 {d.streakDays} 天</span>
            <span>今日 {d.todayMin}m</span>
          </div>
        </div>
      ),
    },
      ...
      render: (d) => (
        <div>
          <div
            ...
            style={{ color: "#94a3b8" }}
          >
            <span>{d.pathNodes} 知识点</span>
            <span>{Math.round(d.pathProgress)}%</span>
          </div>
          <Progress
            percent={Math.round(d.pathProgress)}
            showInfo={false}
        ...
        return (
          <div>
            ...
            >
              {data.map((c, i) => (
                <span key={c.full}>
                  <span style={{ color: colors[i] }}>●</span> {c.done}/{c.total}
                </span>
              ))}
            </div>
          </div>
      ...
      render: (d) => {
        ...
        return (
          <div>
            ...
            >
              <span>
                Lv.{lv.level} {lv.level_name}
              </span>
              <span>
                {lv.current_xp}/{lv.xp_per_level}
              </span>
            </div>
            <div
    ...
    },
    {
      ...
      render: (d) => (
        <div className="space-y-1">
          {(d.tasks.length ? d.tasks : [{ title: "暂无待办任务" }])
            .slice(0, 4)
            .map((t, i) => (
              <div
                key={i}
...
const NeoConsole: React.FC<{ data: HoloData }> = ({ data }) => {
  ...
  const [showHint, setShowHint] = useState(
    () => !localStorage.getItem("hint_seen"),
  );
  useEffect(() => {
    if (!showHint) return;
    const t = setTimeout(() => {
  ...
  const masteryPercent = Math.min(
    100,
    Math.round((data.masteredKps / 20) * 100),
  );
  ...
  return (
    ...
    >
      ...
      {showHint && (
        <div
          className="absolute top-3 left-4 z-20 pointer-events-none"
      ...
      <Canvas
        camera={{ position: [0, 0.1, 16], fov: 44 }}
        dpr={[1, 2]}
        gl={{ antialias: true }}
      >
        {/* 背景：深蓝黑 */}
        ...
        <ParticleNetwork count={180} linkDistance={1.5} radius={8} />
