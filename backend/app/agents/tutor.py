...
class TutorAgent(BaseAgent):
    ...
    async def process(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if task == "answer_question":
                result = await self._socratic_answer(question, history, context.get("profile", {}))
            elif task == "hint":
                result = await self._give_hint(question, history)
            elif task == "encourage":
                result = await self._encourage(history)
            else:
    ...
    async def _socratic_answer(self, question: Union[str, List[Dict[str, Any]]], history: List[Dict[str, Any]], profile: Dict[str, Any]) -> Dict[str, Any]:
        ...
        answer = await self.llm.ainvoke(messages, temperature=0.6, max_tokens=1024)
        # 更新历史
    ...
    async def _give_hint(self, question: str, history: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        hint = await self.llm.ainvoke(messages, temperature=0.5)
        return {"status": "success", "answer": hint, "type": "hint"}

    async def _encourage(self, history: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        text = await self.llm.ainvoke(messages, temperature=0.7)
